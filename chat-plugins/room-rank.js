...
exports.commands = {
	...
	roomleader: function (target, room, user) {
		...
		if (!room.founder) return this.sendReply('The room needs a room founder before it can have a room owner.');
		if (room.founder !== user.userid && !this.can('roommod')) return this.sendReply('/roomowner - Access denied.');
	...
	deroomleader: function (target, room, user) {
		...
		if (!room.founder || user.userid !== room.founder && !this.can('roommod', null, room)) return false;
