...
Config.chatfilter = function (message, user, room, connection, targetUser) {
	...
	for (let x in bannedMessages) {
		if (message.toLowerCase().indexOf(bannedMessages[x]) > -1 && bannedMessages[x] !== '' && message.substr(0, 1) !== '/') {
			...
			Destiny.pmUpperStaff(user.name + ' has been automatically locked for sending a message containing a banned word' +
	...
	if (!user.can('hotpatch') && (pre_matches >= 1 && final_check === 0 || pre_matches >= 2 && final_check >= 1 || message.match(adRegex))) {
		...
		if (!Users.ShadowBan.checkBanned(user)) {
			...
			Destiny.pmUpperStaff(user.name + " has been sbanned for attempting to advertise" + (room ? ". **Room:**" + room.id : " in a private message to " + targetUser.name + ".") + " **Message:** " + message, "~Server");
...
Config.namefilter = function (name, user) {
	let badNames = Config.bannedNames, badHosts = Object.keys(Destiny.lockedHosts), nameId = toId(name);
	...
	Dnsbl.reverse(ip).then(host => {
		...
		if (Destiny.userData[toId(name)] && Destiny.userData[toId(name)].proxywhitelist) return;
 ...
 * Hostfilter Magic *
 * ******************/
Destiny.lockedHosts = Object.create(null);

function loadHostBlacklist() {
	fs.readFile('config/lockedhosts.json', 'utf8', function (err, file) {
		if (err) return;
		Destiny.lockedHosts = JSON.parse(file);
	});
}
loadHostBlacklist();

function saveHost() {
	fs.writeFileSync('config/lockedhosts.json', JSON.stringify(Destiny.lockedHosts));
}
...
exports.commands = {
	lockhost: function (target, room, user) {
		...
		if (Destiny.lockedHosts[target]) return this.errorReply("The host '" + target + "' is was already locked by " + Destiny.lockedHosts[target].by + ".");

		Destiny.lockedHosts[target] = {
			by: user.name,
			on: Date.now(),
		};
		saveHost();
	...
	unlockhost: function (target, room, user) {
		...
		if (!Destiny.lockedHosts[target]) return this.errorReply("The host '" + target + "' is not currently blacklisted.");

		delete Destiny.lockedHosts[target];
		saveHost();
	...
	proxylist: function (target, room, user) {
		if (!this.can('pban')) return false;
		let badHosts = Object.keys(Destiny.lockedHosts);
		...
		badHosts.forEach(proxy => {
			buff += '<tr><td>' + proxy + '</td><td>' + Destiny.nameColor(Destiny.lockedHosts[proxy].by, false) + '</td><td>' + Tools.toDurationString(Date.now() - Destiny.lockedHosts[proxy].on) + ' ago</td></tr>';
		});
